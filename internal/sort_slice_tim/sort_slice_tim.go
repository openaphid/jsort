...
func Sort(a SliceInterface, compare CompareFunc) {
	sort(a, 0, a.len(), compare, nil, 0, 0)
}
 ...
 */
func newTimSort(a SliceInterface, c CompareFunc, work SliceInterface, workBase int, workLen int) *TimSort {
	this := &TimSort{
	...
	if work == nil || workLen < tlen || workBase+tlen > work.len() {
		this.tmp = a.make(tlen)
		this.tmpBase = 0
		this.tmpLen = tlen
	} else {
		this.tmp = work
 ...
 * @since 1.8
 */
func sort(a SliceInterface, lo int, hi int, c CompareFunc, work SliceInterface, workBase int, workLen int) {
 ...
 */
func binarySort(a SliceInterface, lo int, hi int, start int, c CompareFunc) {
	...
	for ; start < hi; start++ {
		...
		default:
			a.slice(left + 1).copy(a.slice2(left, left+n)) // XXX, maybe incorrect
 ...
 *          the specified array
 */
func countRunAndMakeAscending(a SliceInterface, lo int, hi int, c CompareFunc) int {
	//assert lo < hi;
	var runHi = lo + 1
	if runHi == hi {
		return 1
	...
	runHi++
	if a.cmp(c, runHi-1, lo) < 0 { // Descending
		for runHi < hi && a.cmp(c, runHi, runHi-1) < 0 {
			runHi++
		}
 ...
 */
func reverseRange(a SliceInterface, lo int, hi int) {
	hi--
	for lo < hi {
		var t = a.get(lo)
		a.set(lo, a.get(hi))
...
func (this *TimSort) mergeAt(i int) {
	 ...
	 */
	var k = gallopRight(this.a.get(base2), this.a, base1, len1, 0, this.c)
	//assert k >= 0;
	base1 += k
	len1 -= k
	if len1 == 0 {
	 ...
	 */
	len2 = gallopLeft(this.a.get(base1+len1-1), this.a, base2, len2, len2-1, this.c)
	//assert len2 >= 0;
	if len2 == 0 {
		return
	}
 ...
 *    should follow it.
 */
func gallopLeft(key interface{}, a SliceInterface, base int, len int, hint int, c CompareFunc) int {
	//assert len > 0 && hint >= 0 && hint < len;
	var lastOfs = 0
	var ofs = 1
	if c(key, a.get(base+hint)) > 0 {
		// Gallop right until a[base+hint+lastOfs] < key <= a[base+hint+ofs]
		var maxOfs = len - hint
		for ofs < maxOfs && c(key, a.get(base+hint+ofs)) > 0 {
			lastOfs = ofs
			ofs = (ofs << 1) + 1
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRight(key interface{}, a SliceInterface, base int, len int, hint int, c CompareFunc) int {
	//assert len > 0 && hint >= 0 && hint < len;
...
func (this *TimSort) mergeLo(base1 int, len1 int, base2 int, len2 int) {
	...
	len2--
	if len2 == 0 {
		a.slice(dest).copy(tmp.slice2(cursor1, cursor1+len1))
		//System.arraycopy(tmp, cursor1, a, dest, len1);
		return
	}
	if len1 == 1 {
		a.slice(dest).copy(a.slice2(cursor2, cursor2+len2))
...
outer:
	for {
		...
		for {
			...
			if c(a.get(cursor2), tmp.get(cursor1)) < 0 {
				a.set(dest, a.get(cursor2))
				dest++
				cursor2++
		...
		for {
			...
			if count1 != 0 {
				a.slice(dest).copy(tmp.slice2(cursor1, cursor1+count1))
			...
			if count2 != 0 {
				a.slice(dest).copy(a.slice2(cursor2, cursor2+count2))
...
func (this *TimSort) mergeHi(base1, len1, base2, len2 int) {
	...
	var a = this.a // For performance
	var tmp SliceInterface = this.ensureCapacity(len2)
	var tmpBase = this.tmpBase

	tmp.slice(tmpBase).copy(a.slice2(base2, base2+len2))
	...
	len1--
	if len1 == 0 {
		a.slice(dest - (len2 - 1)).copy(tmp.slice2(tmpBase, tmpBase+len2))
	...
	if len2 == 1 {
		dest -= len1
		cursor1 -= len1
		a.slice(dest + 1).copy(a.slice2(cursor1+1, cursor1+1+len1))
...
outer:
	for true {
		...
		for {
			...
			if c(tmp.get(cursor2), a.get(cursor1)) < 0 {
				a.set(dest, a.get(cursor1))
				dest--
				cursor1--
		...
		for {
			...
			if count1 != 0 {
				...
				cursor1 -= count1
				len1 -= count1
				a.slice(dest + 1).copy(a.slice2(cursor1+1, cursor1+1+count1))
			...
			if count2 != 0 {
				...
				cursor2 -= count2
				len2 -= count2
				a.slice(dest + 1).copy(tmp.slice2(cursor2+1, cursor2+1+count2))
 ...
 */
func (this *TimSort) ensureCapacity(minCapacity int) SliceInterface {
	if this.tmpLen < minCapacity {
		...
		if newSize < 0 { // Not bloody likely!
			...
			newSize = min(newSize, this.a.len()>>1)
		}

		this.tmp = this.a.make(newSize)
		this.tmpLen = newSize
		this.tmpBase = 0
	}
	return this.tmp
