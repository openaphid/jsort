...
func BuiltinSortSpecializedInts(data []primitive) {
	n := len(data)
...
func quickSort(data []primitive, a, b, maxDepth int) {
...
func heapSort(data []primitive, a, b int) {
	first := a
	lo := 0
	hi := b - a
...
func siftDown(data []primitive, lo, hi, first int) {
	root := lo
	for {
		child := 2*root + 1
		if child >= hi {
...
func doPivot(data []primitive, lo, hi int) (midlo, midhi int) {
...
func medianOfThree(data []primitive, m1, m0, m2 int) {
	// sort 3 elements
	if data[m1] < data[m0] {
...
func insertionSort(data []primitive, a, b int) {
	for i := a + 1; i < b; i++ {
